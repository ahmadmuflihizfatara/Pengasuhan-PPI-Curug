...
.greeting-banner::before { content: ''; position: absolute; right: -60px; top: -60px; width: 220px; height: 220px; background: rgba(255,255,255,.06); border-radius: 50%; }
.greeting-banner::after  { content: ''; position: absolute; right: 100px; bottom: -80px; width: 180px; height: 180px; background: rgba(253,187,17,.08); border-radius: 50%; }
...
.greeting-badge {
    ...
    background: rgba(253,187,17,.15);
    border-radius: 16px; padding: 16px 22px;
    text-align: center; backdrop-filter: blur(4px);
    border: 1px solid rgba(253,187,17,.25);
}
.greeting-badge .ava { width: 52px; height: 52px; border-radius: 50%; background: rgba(253,187,17,.25); display: flex; align-items: center; justify-content: center; font-size: 20px; font-weight: 800; margin: 0 auto 8px; }
...
.stat-card:hover { transform: translateY(-3px); border-color: #fdbb11; color: inherit; }
...
.stat-change.neutral { color: #6b7c93; }
...
.section-link   { font-size: 12px; color: #fdbb11; font-weight: 600; text-decoration: none; }
.section-link:hover { text-decoration: underline; }

/* === CARD === */
.card      { background: white; border-radius: 16px; border: 1px solid #d4dbe5; }
.card-body { padding: 20px; }

/* === ACARA CARDS === */
.acara-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; margin-bottom: 24px; }
.acara-card { background: white; border-radius: 14px; overflow: hidden; border: 1px solid #d4dbe5; transition: transform .2s; }
...
.acara-card-desc  { font-size: 11px; color: #6b7c93; line-height: 1.4; }

/* === TABLES === */
.surat-table table, .acara-table table { width: 100%; border-collapse: collapse; }
.surat-table th, .acara-table th { padding: 12px 16px; text-align: left; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: .06em; color: #6b7c93; background: #f8f9fb; }
.surat-table td, .acara-table td { padding: 12px 16px; font-size: 13px; color: #444; border-top: 1px solid #eef3f9; }
.surat-table tr:hover td, .acara-table tr:hover td { background: #f8f9fb; }
.status-badge { display: inline-flex; align-items: center; padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 700; }
.time-badge   { background: #fff8e1; color: #12283a; padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 700; }

/* === EMPTY === */
.empty-mini { text-align: center; padding: 28px 16px; color: #6b7c93; }
    ...
    <div class="main-content">
        ...
        <div class="stats-grid" style="grid-template-columns:repeat(2,1fr);">
            <div class="stat-card" style="cursor:default;">
                <div class="stat-icon" style="background:#12283a;">
            ...
            <a href="{{ route('poin.index') }}" class="stat-card">
                <div class="stat-icon" style="background:#fdbb11;">
        ...
        @endphp
        <div style="background: #12283a; border: 1px solid #d4dbe5; border-radius: 16px; padding: 20px 24px; margin-bottom: 24px; color: white; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 16px;">
            <div style="display: flex; align-items: center; gap: 16px;">
                <div style="width: 50px; height: 50px; border-radius: 14px; background: rgba(253,187,17,.15); border: 1px solid #fdbb11; display: flex; align-items: center; justify-content: center; font-size: 22px; color: #fdbb11;">
                    <i class="fas fa-walking"></i>
                </div>
                <div>
                    <div style="font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 1px; color: rgba(255,255,255,.6);">Log Pergerakan Taruna (Sistem Pos Jaga)</div>
                    <div style="font-size: 18px; font-weight: 800; color: white; margin-top: 2px;">
                        ...
                        <span style="font-size: 13px; color: rgba(255,255,255,.6); font-weight: 500; margin-left: 8px;">(🟢 {{ $kembaliHariIni }} kembali hari ini)</span>
                    </div>
                </div>
            </div>
            <div style="display: flex; gap: 10px;">
                <a href="{{ route('log-pergerakan.tablet') }}" style="background: #fdbb11; color: #12283a; border-radius: 10px; padding: 10px 18px; font-size: 13px; font-weight: 700; text-decoration: none; display: inline-flex; align-items: center; gap: 8px; transition: background 0.2s;">
                    <i class="fas fa-tablet-alt"></i> Mode Tablet Pos Jaga
                </a>
                <a href="{{ route('log-pergerakan.tv') }}" target="_blank" style="background: rgba(255,255,255,.1); color: white; border: 1px solid rgba(255,255,255,.25); border-radius: 10px; padding: 10px 18px; font-size: 13px; font-weight: 700; text-decoration: none; display: inline-flex; align-items: center; gap: 8px;">
        ...
        <div class="section-header">
            <h3 class="section-title">
                <i class="fas fa-calendar-star" style="color:#fdbb11;margin-right:8px;"></i>
        ...
        <div class="card" style="margin-bottom:24px;">
            <div class="card-body">
                <div class="empty-mini">
                    <i class="fas fa-calendar-times" style="color:#d4dbe5;"></i>
                    <p>Belum ada acara dijadwalkan.
                        <a href="{{ route('acara.create') }}" style="color:#fdbb11;font-weight:600;">+ Tambah Acara</a>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="acara-grid" style="margin-bottom:24px;">
            @php
                $acara_colors = [
                    ['#12283a','#1e3a52'],['#fdbb11','#d4a10e'],
                    ['#38a169','#2f855a'],['#12283a','#fdbb11'],
                ];
                $ci = 0;
            @endphp
        ...
        <div class="two-col">
            ...
            <div>
                <div class="section-header">
                    <h3 class="section-title">
                        <i class="fas fa-envelope-open-text" style="color:#fdbb11;margin-right:8px;"></i>Surat Terbaru
                    </h3>
                ...
                <div class="card surat-table">
                    ...
                    <div class="card-body">
                        <div class="empty-mini">
                            <i class="fas fa-inbox" style="color:#d4dbe5;"></i>
                            <p>Belum ada surat.
                                <a href="{{ route('surat.create') }}" style="color:#fdbb11;font-weight:600;">+ Tambah</a>
                            </p>
                        </div>
                    </div>
                    @else
                        ...
                        <tbody>
                            ...
                            <tr onclick="window.location='{{ route('surat.show', $s->id) }}'" style="cursor:pointer;">
                                <td>
                                    ...
                                    <div style="font-size:11px;color:#6b7c93;">{{ $s->pengirim }}</div>
                                </td>
                                <td>
                                    <span style="background:#fff8e1;color:#12283a;padding:2px 8px;border-radius:20px;font-size:10px;font-weight:700;white-space:nowrap;">
            ...
            <div>
                <div class="section-header">
                    <h3 class="section-title">
                        <i class="fas fa-calendar-alt" style="color:#12283a;margin-right:8px;"></i>Jadwal Acara
                    </h3>
                            ...
                            <tr>
                                <td>
                                    <div style="display:flex;align-items:center;gap:8px;">
                                        <div style="width:28px;height:28px;border-radius:8px;background:#12283a;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
        ...
        <div class="section-header">
            <h3 class="section-title">
                <i class="fas fa-bolt" style="color:#fdbb11;margin-right:8px;"></i>Aksi Cepat
            </h3>
        </div>
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:28px;">
            <a href="{{ route('surat.create') }}" style="background:#fdbb11;color:#12283a;border-radius:14px;padding:18px;text-decoration:none;display:flex;align-items:center;gap:12px;font-weight:700;font-size:13px;transition:transform .15s;" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform=''">
                <i class="fas fa-file-alt" style="font-size:20px;opacity:.9;"></i>
                <div><div style="font-size:11px;opacity:.7;font-weight:400;">Buat</div>Surat Baru</div>
            </a>
            <a href="{{ route('acara.create') }}" style="background:#12283a;color:white;border-radius:14px;padding:18px;text-decoration:none;display:flex;align-items:center;gap:12px;font-weight:700;font-size:13px;transition:transform .15s;" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform=''">
                <i class="fas fa-calendar-plus" style="font-size:20px;opacity:.9;"></i>
                <div><div style="font-size:11px;opacity:.7;font-weight:400;">Tambah</div>Acara</div>
            </a>
            <a href="{{ route('poin.index') }}" style="background:#12283a;color:#fdbb11;border-radius:14px;padding:18px;text-decoration:none;display:flex;align-items:center;gap:12px;font-weight:700;font-size:13px;transition:transform .15s;" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform=''">
                <i class="fas fa-star" style="font-size:20px;opacity:.9;"></i>
                <div><div style="font-size:11px;opacity:.7;font-weight:400;">Kelola</div>Poin Mahasiswa</div>
            </a>
            <a href="{{ route('mahasiswa.index') }}" style="background:#fdbb11;color:#12283a;border-radius:14px;padding:18px;text-decoration:none;display:flex;align-items:center;gap:12px;font-weight:700;font-size:13px;transition:transform .15s;" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform=''">
                <i class="fas fa-users" style="font-size:20px;opacity:.9;"></i>
                <div><div style="font-size:11px;opacity:.7;font-weight:400;">Lihat</div>Database Mahasiswa</div>
            </a>
        </div>
        @endif
